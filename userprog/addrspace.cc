...
void
AddrSpace::RequestFrames(int nFrames)
{
    FrameListNode *head = kernel->freeFrames->AllocateFrames(nFrames);
    for (int i = 0; i < nFrames; i++) {
        pageTable[i].physicalPage = head->frameNo;
        pageTable[i].valid = TRUE;
        head = head->next;
    }
}
...
// how big is address space?
    ...
    //cout << "Initializing address space: " << numPages << ", " << size << "\n";

// then, copy in the code and data segments into memory
// Note: this code assumes that virtual address = physical address
    if (noffH.code.size > 0) {
        unsigned int codePhyAddr;
        Translate(noffH.code.virtualAddr, &codePhyAddr, 1);
        DEBUG(dbgAddr, "Initializing code segment.");
	DEBUG(dbgAddr, noffH.code.virtualAddr << ", " << noffH.code.size);
        executable->ReadAt(
		&(kernel->machine->mainMemory[codePhyAddr]), 
			noffH.code.size, noffH.code.inFileAddr);
    }
    if (noffH.initData.size > 0) {
        unsigned int dataPhyAddr;
        Translate(noffH.initData.virtualAddr, &dataPhyAddr, 1);
        DEBUG(dbgAddr, "Initializing data segment.");
	DEBUG(dbgAddr, noffH.initData.virtualAddr << ", " << noffH.initData.size);
        executable->ReadAt(
		&(kernel->machine->mainMemory[dataPhyAddr]),
