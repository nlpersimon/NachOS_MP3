...
FreeFrames::FreeFrames(int nFreeFrames)
{
    nFreeFrames = nFreeFrames;

    FrameListNode *curTail = new FrameListNode();
    curTail->frameNo = 0;
    curTail->next = curTail;
    for (int i = 1; i < nFreeFrames; i++) {
        FrameListNode *newTail = new FrameListNode();
        newTail->frameNo = i;
    ...
    head = curTail->next;
	DEBUG(dbgSys, "head frame no." << head->frameNo);    
    curTail->next = NULL;
}

FreeFrames::~FreeFrames()
{
    FrameListNode *next = head;
    while (next) {
        next = next->next;
        delete next;
    }
...
FrameListNode*
FreeFrames::AllocateFrames(int nFrames)
{
    ASSERT(nFreeFrames >= nFreeFrames);
    FrameListNode *allocHead = head;
    FrameListNode *curTail = head;
    for (int i = 1; i < nFrames; i++) {
        curTail = curTail->next;
    }
    head = curTail->next;
    curTail->next = allocHead;
    nFreeFrames = nFreeFrames - nFrames;
    return allocHead;
}

void
FreeFrames::ReleaseFrames(FrameListNode* head, int nFrames)
{
    FrameListNode *secondNode = head->next;
    head->next = this->head;
...
Kernel::Initialize()
{
    ...
    freeFrames = new FreeFrames(NumPhysPages);
#ifdef FILESYS_STUB
...
int Kernel::Exec(char* name)
{
    ...
    t[threadNum]->setPriority(threadPriority[threadNum]);
