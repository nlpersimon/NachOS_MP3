...
typedef struct _frame_list_node_ {
  int frameNo;
  struct _frame_list_node_ *next;
} FrameListNode;

class FreeFrames {
  public:
    FreeFrames(int nFreeFrames);
    ~FreeFrames();

    FrameListNode* AllocateFrames(int nFrames);
    void ReleaseFrames(FrameListNode* head, int nFrames);
